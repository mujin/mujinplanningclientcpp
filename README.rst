...
By default, library is installed in ``C:\Program Files\mujinplanningclient``.
...
1. To dynamically load all the DLLs at run time, link with ``lib\mujinplanningclient-vcXX-mt.lib``. Then need to add ``C:\Program Files\mujinplanningclient\bin`` to the PATH, or register all the DLLs with windows system. Need to define the following flags::
...
**Note:** Several examples require the files inside ``C:\Program Files\mujinplanningclient\share`` folder.
...
4. Open the **build/mujinplanningclientcpp.sln** solution and compile the **ALL_BUILD** project.
...
5. To build the project using the Visual Studio Command Prompt
  ::
     ...
     msbuild mujinplanningclient.sln /p:Configuration=Release
